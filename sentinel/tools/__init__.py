...
class ToolAdapter(ABC):
    ...
    async def _rate_limit_acquire(self) -> tuple[str, str, int, float] | None:
        ...
        try:
            from sentinel.config import get_config
            from sentinel.core.concurrency import tool_bucket
    ...
    async def _run_subprocess(
        ...
        if missing is not None:
            return missing
        from sentinel.core.cache import cache_get, cache_key, cache_put, ttl_for
...
def load_adapters(registry: ToolRegistry) -> None:
    ...
    adapters shipped by SENTINEL (recon, web, api, etc.).
    """
