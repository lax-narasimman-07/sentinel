"""Uniform error contract and guarded-tool decorator for SENTINEL.
...
def _audit_log_path() -> str:
    ...
    if _audit_path:
        return _audit_path
    try:
        base = get_config().base_dir or str(Path.home() / ".sentinel")
    except Exception:
        base = os.environ.get("SENTINEL_BASE_DIR", str(Path.home() / ".sentinel"))
