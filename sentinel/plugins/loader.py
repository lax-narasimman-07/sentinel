...
from sentinel.plugins.base import Plugin, PluginState
...
def _load_entry_point_plugins(registry: PluginRegistry, config: dict[str, Any] | None) -> None:
    try:
        ...
        eps = entry_points()
        sentinel_eps = eps.select(group="sentinel.plugins") if hasattr(eps, "select") else list(eps.get("sentinel.plugins", []))
