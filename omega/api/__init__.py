...
class APISecurityEngine:
                ...
                if resp.get("status_code") == 200:
                    ...
                    try:
                        ...
                        if "openapi" in spec or "swagger" in spec:
                            return {"found": True, "url": url, "spec": spec, "version": spec.get("openapi", spec.get("swagger", "unknown"))}
    ...
    async def infer_endpoints(self, base_url: str) -> dict[str, Any]:
    ...
    async def analyze_authentication(self, url: str) -> dict[str, Any]:
        ...
        cors_resp = await self.http.request("GET", url, headers={"Origin": "https://evil.com"})
    ...
    async def test_idor(self, url_pattern: str, id_values: list[int | str] | None = None, engagement_id: str = "") -> dict[str, Any]:
    ...
    async def analyze_graphql_introspection(self, graphql_url: str) -> dict[str, Any]:
        ...
        try:
            resp = await self.http.post(graphql_url, json_body={"query": "{ __schema { types { name fields { name type { name kind ofType { name } } } } } }"})
            if resp.get("status_code") == 200:
                ...
                if "data" in data and data["data"].get("__schema"):
                    ...
                    result["types"] = [{"name": t["name"], "fields": [f["name"] for f in t.get("fields", [])]} for t in types if not t["name"].startswith("__")]
    ...
    async def full_scan(self, target: str, engagement_id: str = "") -> dict[str, Any]:
        ...
        results["openapi"] = await self.discover_openapi(url)
        results["graphql"] = await self.discover_graphql(url)
        results["auth"] = await self.analyze_authentication(url)
        results["endpoints"] = await self.infer_endpoints(url)
        return results
...
try:
    ...
    logger.warning(f"Could not load API routes: {type(e).__name__}: {e}")
...
def broadcast_event(event_type: str, data: dict) -> None:
    ...
    if event_type == "finding":
        ...
        if len(_dashboard_state["logs"]) > 500:
            _dashboard_state["logs"] = _dashboard_state["logs"][-500:]
