"""CLI entry point for OMEGA-CYBER-MCP."""
...
from omega.config import OmegaConfig
from omega.tools import ToolRegistry, load_adapters


@click.group()
@click.version_option(__version__, prog_name="omega")
...
def main(verbose: bool, log_level: str) -> None:
    """OMEGA — AI-Assisted Security Research Platform."""
...
def serve(host: str, port: int) -> None:
    ...
    from omega.mcp import create_server
    config = OmegaConfig()
...
def start(dashboard_host: str, dashboard_port: int, mcp_host: str, mcp_port: int) -> None:
    """Start OMEGA platform (dashboard + MCP server)."""
    click.echo(f"⚡ Starting OMEGA Security Platform v{__version__}")
...
def doctor() -> None:
    ...
    click.echo("🔍 OMEGA System Health Check")
    click.echo("=" * 50)
    ...
    click.echo("=" * 50)
    click.echo("Ready to launch: omega start")
