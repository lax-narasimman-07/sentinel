...
Covers the newly exposed tool families:
  ...
  - Web engine:           sentinel_web_jwt, sentinel_web_tech
  - API security engine:  sentinel_api_openapi/graphql/auth/idor/introspection/full_scan
...
from sentinel.core.schemas import Severity, new_id, now_utc
...
NEW_TOOLS = [
    ...
    "sentinel_web_jwt",
    "sentinel_web_tech",
    "sentinel_api_openapi",
    "sentinel_api_graphql",
    "sentinel_api_auth",
    "sentinel_api_idor",
    "sentinel_api_introspection",
    "sentinel_api_full_scan",
    "sentinel_recon_vuln_scan",
    "sentinel_recon_server_audit",
    "sentinel_recon_dirbrute",
    "sentinel_recon_webcrawl",
    "sentinel_recon_port_rapid",
    "sentinel_recon_fastportscan",
    "sentinel_recon_dns_lookup",
    "sentinel_recon_waf_detect",
    "sentinel_ctf_resolve_hypothesis",
    "sentinel_ctf_add_note",
    "sentinel_ctf_add_artifact",
    "sentinel_ctf_hypothesis_ledger",
]

# Active tools: deny by default when an engagement has no scope rules.
ACTIVE_TOOLS = [
    ("sentinel_recon_vuln_scan", {"target": "http://active.example.com"}),
    ("sentinel_recon_server_audit", {"target": "http://active.example.com"}),
    ("sentinel_recon_dirbrute", {"target": "http://active.example.com"}),
    ("sentinel_recon_port_rapid", {"target": "127.0.0.1"}),
    ("sentinel_recon_fastportscan", {"target": "127.0.0.1"}),
    ("sentinel_api_idor", {"url_pattern": "http://api.example.com/users/{id}", "id_values": "1,2"}),
    ("sentinel_api_full_scan", {"target": "http://api.example.com"}),
]

# Passive tools: permitted under analysis_only (observation-only).
PASSIVE_TOOLS = [
    ("sentinel_recon_webcrawl", {"target": STEALTH}),
    ("sentinel_recon_dns_lookup", {"target": "example.com"}),
    ("sentinel_recon_waf_detect", {"target": "http://example.com"}),
    ("sentinel_web_jwt", {"url": STEALTH}),
    ("sentinel_web_tech", {"url": STEALTH}),
    ("sentinel_api_openapi", {"base_url": STEALTH}),
    ("sentinel_api_graphql", {"base_url": STEALTH}),
    ("sentinel_api_auth", {"url": STEALTH}),
    ("sentinel_api_introspection", {"graphql_url": STEALTH + "/graphql"}),
]
...
async def fresh_server() -> AsyncGenerator[ClientSession, None]:
    with tempfile.TemporaryDirectory(prefix="sentinel_ph3_") as tmpdir:
...
async def test_active_tools_deny_without_scope_rules():
    ...
    async with fresh_server() as s:
        eng = await _call(s, "sentinel_engagement_create", {"name": "P3-A", "mode": "pentest"})
        eid = _json(eng)["id"]
...
async def test_active_tools_deny_analysis_only():
    ...
    async with fresh_server() as s:
        eng = await _call(s, "sentinel_engagement_create", {"name": "P3-AO", "mode": "analysis_only"})
        eid = _json(eng)["id"]
        for name, args in ACTIVE_TOOLS:
            r = await _call(s, name, {**args, "engagement_id": eid})
...
async def test_passive_tools_allowed_analysis_only():
    ...
    async with fresh_server() as s:
        eng = await _call(s, "sentinel_engagement_create", {"name": "P3-P", "mode": "analysis_only"})
        eid = _json(eng)["id"]
...
async def test_missing_binary_tools_return_structured_error():
    ...
    async with fresh_server() as s:
        r = await _call(s, "sentinel_recon_dns_lookup", {"target": "example.com"})
...
async def test_ctf_toolkit_hypothesis_lifecycle():
    async with fresh_server() as s:
        eng = await _call(s, "sentinel_engagement_create", {"name": "P3-CTF", "mode": "ctf"})
        eid = _json(eng)["id"]
        ch = await _call(s, "sentinel_ctf_challenge_create", {"engagement_id": eid, "name": "Rev Me", "category": "rev"})
        challenge = _json(ch)
        assert isinstance(challenge, dict)
        cid = challenge["id"]

        hyp = await _call(s, "sentinel_ctf_hypothesis", {"challenge_id": cid, "hypothesis": "flag in constant pool"})
        hyp_data = _json(hyp)
        hyp_id = hyp_data["id"]

        r = await _call(s, "sentinel_ctf_resolve_hypothesis", {
            "challenge_id": cid, "hypothesis_id": hyp_id,
            "result": "binary strings", "successful": True,
        })
        assert _json(r).get("resolved") is True

        r = await _call(s, "sentinel_ctf_add_note", {"challenge_id": cid, "note": "check .rodata"})
        assert _json(r).get("added") is True

        r = await _call(s, "sentinel_ctf_add_artifact", {"challenge_id": cid, "artifact": "/bin/ls"})
        assert _json(r).get("added") is True

        ledger = await _call(s, "sentinel_ctf_hypothesis_ledger", {"challenge_id": cid})
        data = _json(ledger)
...
async def test_report_includes_hypotheses():
    async with fresh_server() as s:
        eng = await _call(s, "sentinel_engagement_create", {"name": "P3-RPT", "mode": "ctf"})
        eid = _json(eng)["id"]
        await _call(s, "sentinel_hypothesis_create", {
            "engagement_id": eid, "category": "web", "target": "http://t.example.com",
            "hypothesis": "IDOR on items endpoint", "endpoint": "/items",
        })
        md = await _call(s, "sentinel_report_generate", {"engagement_id": eid, "format": "markdown"})
        assert "## Hypotheses" in _extract(md)

        js = await _call(s, "sentinel_report_generate", {"engagement_id": eid, "format": "json"})
        data = _json(js)
...
async def test_orchestrator_api_engine_wired():
    with tempfile.TemporaryDirectory(prefix="sentinel_ph3_db_") as td:
...
async def test_triage_nuclei_findings_creates_records():
    ...
    with tempfile.TemporaryDirectory(prefix="sentinel_triage_") as td:
        db = Database(os.path.join(td, "test.db"))
        await db.connect()
        try:
            orch = Orchestrator(db)
        ...
        try:
            ...
            class FakeNucleiAdapter:
                ...
                async def execute(self, request):
                    from sentinel.core.schemas import ToolResult
                    return ToolResult(
...
async def test_triage_deduplicates():
    ...
    with tempfile.TemporaryDirectory(prefix="sentinel_triage_") as td:
        ...
        try:
            ...
            findings = await orch.findings.list_findings(eng.id)
            from sentinel.core.schemas import ValidationStatus
...
# ═══════════════════════════════════════════════════════════════════════════
# Phase 4 — sentinel_health_check
# ═══════════════════════════════════════════════════════════════════════════


@pytest.mark.asyncio
async def test_health_check_reports_status():
    """sentinel_health_check returns structured health with DB + tool registrations."""
    async with fresh_server() as s:
        r = await _call(s, "sentinel_health_check", {})
        data = _json(r)
